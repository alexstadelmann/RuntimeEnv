...
push (CHP a) (st, cod, env, reg, tr, us) =
...
call stor@(st, cod, env, reg, tr, us)
  | numAt st (c reg) == (-1) && numAt st (c reg + 6)==0 = -- case: there are no clauses and atom is positive.
    let reg' = reg {b = True,
                    p = cLast env}
    in (st, cod, env, reg', tr, us)
  | numAt st (c reg) == (-1) = 
...
returnL stor@(st, cod, env, reg, tr, us)
  | numAt st (r reg + 5) == 1 = -- uppermost atom in stack is negative and still a resolution succeeded, thus the next command(=backtrack) is called with the b-flag on.
    let reg' = reg {p = p reg + 1,
                    b = True}
        st' = setCNil stor
    in (st', cod, env, reg', tr, us)

  | numAt st (r reg + 4) /= l reg - 1 = --stay on same proof tree level
...
backtrack stor@(st, cod, env, reg, tr, us) 
  | b reg = 
    case (numAt st (c reg + 6), numAt st (c reg),  numAt st (r reg), cod !! (p reg - 1)) of
      (1, -1, _, Unify s) ->  let reg' = reg{p = numAt st (r reg + 1), -- negative atom "successfully" failed, that is the resolution (of its negation) with all clause-heads failed.
                                             b = False,
                                             e = numAt st $ r reg + 2}
                                  st' = setNProven stor
                              in (st', cod, env, reg', tr, us)
      ...
      (_, -1, _, _) ->  let newC = numAt st $ r reg
                            reg' = reg {c = newC,
                                        r = newC + 1}
...
setCNil (st, _, _, reg, _, _) =
  let pos = length st - c reg - 1
  in take pos st ++ NUM (-1): drop (pos + 1) st

setNProven :: Storage -> Stack
setNProven (st, _, _, reg, _, _) =
  let pos = length st - (r reg + 5) - 1
  in take pos st ++ NUM (-1):drop (pos + 1) st