...
parse s = case program s [] of
               ([], p) -> p
               ([(NewLine,_)], p) -> p
               ((sym,line):_,_) -> error $ "Parse error in line " ++ show line 
                                ++ ".\nParsing of program already finished, but found: " 
                                ++ show sym
...
program ((sym,line):_) _ = error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected start of program clause or goal, but found: " 
                                ++ show sym
...
pclause s@((Name _, _):_) = uncurry pclause' $ nvlterm s where
  ...
  pclause' s@((If,_):_) nvlt =
    let g = goal s
    in uncurry pclause'' (fst g, PClause nvlt $ Just $ snd g) 
  pclause' ((sym,line):_) _ = error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected end of program clause or start of goal, but found: "  
                                ++ show sym

  pclause'':: [(Symbol, Int)] -> PClause -> ([(Symbol, Int)], PClause)          --Jede Klausel muss in eigener,neuen Zeile stehen
  pclause'' ((NewLine,_):t) pc = (t, pc) 
  pclause'' ((sym,line):_) _ =  error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected new line, but found: "  
                                ++ show sym


pclause ((sym,line):_) = error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected start of program clause, but found: " 
                                ++ show sym
...
goal ((If,_):t) = goal' t [] where
  ...
  goal' s@((sym,line):_) ls
    | fstOfLit sym = let l = literal s
                         in goal'' (fst l) $ (snd l):ls
    | otherwise = error $ "Parse error in line " ++ show line
                                ++ ".\nExpected start of literal, but found: "  
                                ++ show sym

  goal'' :: [(Symbol, Int)] -> [Literal] -> ([(Symbol, Int)], Goal)
  goal'' ((And,_):t) ls = goal' t ls
  goal'' ((Point,_):t) ls = (t, Goal $ reverse ls)
  goal'' ((sym,line):_) _ = error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected end of literal or goal, but found: " 
                                ++ show sym

goal ((sym,line):_) = error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected start of goal, but found: "  
                                ++ show sym
...
literal s@((sym,line):_)
  | fstOfLTerm sym = let lt = lterm s
                       in (fst lt, Literal False $ snd lt)
  | otherwise = error $ "Parse error in line " ++ show line
                                ++ ".\nExpected \"not\" or name or variable, but found: " 
                                ++ show sym
...
nvlterm ((Name n,_):t) = nvlterm' t n where
  ...
  nvlterm' s n = (s, NVLTerm n [])              
            -- Fehlermeldung wird von pclause oder von lterm abgefangen

  nvlterm'' :: [(Symbol, Int)] -> String -> [LTerm] -> ([(Symbol, Int)], NVLTerm)
  nvlterm'' s@((sym,line):_) n lts
    | fstOfLTerm sym = let l = lterm s
                         in nvlterm''' (fst l) n $ (snd l):lts
    | otherwise = error $ "Parse error in line " ++ show line
                                ++ ".\nExpected name or variable, but found:" 
                                ++ show sym

  nvlterm''' :: [(Symbol, Int)] -> String -> [LTerm] -> ([(Symbol, Int)], NVLTerm)
  nvlterm''' ((And,_):t) n lts = nvlterm'' t n lts
  nvlterm''' ((RBracket,_):t) n lts = (t, NVLTerm n $ reverse lts)
  nvlterm''' ((sym,line):_) _ _ = error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected end of L-Term, but found: "      
                                ++ show sym

nvlterm ((sym,line):_) = error $ "Parse error in line " ++ show line 
...
lterm ((sym,line):_) = error $ "Parse error in line " ++ show line 
                                ++ ".\nExpected name or variable, but found: " 
                                ++ show sym
